-- Create function to update timestamps
CREATE OR REPLACE FUNCTION public.update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SET search_path = public;

-- Create inventory_checks table for storing vehicle control history
CREATE TABLE public.inventory_checks (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL DEFAULT auth.uid(),
  vehicle_id UUID NOT NULL,
  check_date TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  checked_by TEXT NOT NULL,
  location TEXT,
  items_data JSONB NOT NULL,
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Enable Row Level Security
ALTER TABLE public.inventory_checks ENABLE ROW LEVEL SECURITY;

-- Create RLS policies
CREATE POLICY "Users can view their own inventory checks"
ON public.inventory_checks
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own inventory checks"
ON public.inventory_checks
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own inventory checks"
ON public.inventory_checks
FOR UPDATE
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own inventory checks"
ON public.inventory_checks
FOR DELETE
USING (auth.uid() = user_id);

-- Create trigger for automatic timestamp updates
CREATE TRIGGER update_inventory_checks_updated_at
BEFORE UPDATE ON public.inventory_checks
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();