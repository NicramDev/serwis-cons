...
const EquipmentList = ({
  ...
  return <div className="flex flex-col gap-3">
      {sortedEquipment.map(item => <EquipmentCard key={item.id} equipment={item} actions={<>
              {onViewEquipment && <Button variant="outline" size="sm" onClick={() => onViewEquipment(item)} className="h-7 px-2 text-xs mx-[191px]">
                  <Eye className="h-3.5 w-3.5 mr-1" />
                  Podgląd
                </Button>}
              {onMoveEquipment && <Button variant="outline" size="sm" className="h-7 px-2 text-xs" onClick={e => {
        e.stopPropagation();
        onMoveEquipment(item);
              ...
              {onEditEquipment && <Button variant="outline" size="sm" onClick={() => onEditEquipment(item)} className="h-7 px-2 text-xs mx-[3px]">
                  <Edit className="h-3.5 w-3.5 mr-1" />
                  Edytuj
                </Button>}
              {onDeleteEquipment && <Button variant="destructive" size="sm" className="h-7 px-2 text-xs" onClick={() => onDeleteEquipment(item)}>
